# Team Points — Specifications

## 1. Overview

A points-tracking system for a school (or similar organization). Students are
divided into **teams**. **Teachers** award or deduct points from
teams throughout the day; an **admin** manages the roster of teams and
...
- **admin** — a superset of teacher. Admins can do everything a teacher can,
  **plus** manage teams and teacher accounts, and void *any* teacher's
...
Removing a team or a teacher account is a **soft-delete** (`active =
...
display its name normally in event history. Nothing about a team or teacher
...
| `GET /api/teams` | List active teams with their current point totals. |
| `GET /api/teachers` | List active teacher accounts (`id`, `username`, `display_name`). |
| `GET /api/events?page_size=&before_id=&teacher_id=&team_id=` | Paginated event history, **newest first**. `page_size` default **20**, max **100**. `before_id` (optional) continues from a previous page. Response: `{ events: [...], next_id: <id, or null if no more> }` — `next_id` is the id to pass as `before_id` for the following page. `teacher_id`/`team_id` filters are optional and combine with AND. |
...
| Method & path | Description |
|---|---|
| `POST /api/teams/{teamId}/points` | Body `{ points: <signed non-zero int> }`. Creates a point event; `comment` is generated server-side (see §5.5) and is not a request parameter. |
...
| Method & path | Description |
|---|---|
| `POST /api/teams` | Body `{ name }`. Create a team. |
| `DELETE /api/teams/{teamId}` | Soft-delete (deactivate) a team. |
...
The teacher and team names and the signed point delta are passed to the
...
ajouté/retiré {N} points à {Team}.`") instead of blocking the request. A
...
- **Classement** (public, no auth) — the app's default/start screen, and where
  logout returns to. A grid of team cards (name + points), sortable by name
  or by points, with adjustable columns-per-row and font size (handy when
  projected). Auto-refreshes every minute by re-fetching `GET /api/teams`
...
- **Enseignant** (authenticated, teacher or admin): award or remove points
  from a team via a single signed-amount input behind a confirmation dialog
  — "removing" is just a negative amount on the same
  `POST /api/teams/{teamId}/points` call, not a separate action. Also shows
...
- **Administrateur** (authenticated, admin only): manage the roster of teams
